...
def transfer(agencia_origem, num_conta_origem, agencia_destino, num_conta_destino, valor):
    conta_origem = BuscarContaPorAgenciaUseCase(agencia=agencia_origem, num_conta=num_conta_origem)
    if conta_origem.saldo < valor:
        raise SaldoInsuficiente('Saldo insuficiente para a operação solicitada.')

    conta_destino = BuscarContaPorAgenciaUseCase(agencia=agencia_destino, num_conta=num_conta_destino)
